...
addReviews = async (req, res) => {
  ...
  let requiredFields = ["productId", "reviewerName", "review"];
  let validator = helpers.validateParams(req, requiredFields);
  if (!validator.status) {
    return res.status(203).send({
      status: false,
  ...
  let result = await Vendor.addReviews(req.body,req.decoded.user_id);
  if (result.status) {
...
deleteReview = async (req, res) => {
  let requiredFields = ['reviewId'];
  let validator = helpers.validateParams(req, requiredFields);
  if (!validator.status) {
      return helpers.showOutput(res, helpers.showResponse(false, validator.message), 203);
  }
  
  let result = await Vendor.deleteReview(req.body.reviewId,req.decoded.user_id);
  return helpers.showOutput(res, result, result.code);
}

getReviews = async (req, res) => {
  
  let result = await Vendor.getReviews(req.decoded.user_id);
    return helpers.showOutput(res, result, result.code);
