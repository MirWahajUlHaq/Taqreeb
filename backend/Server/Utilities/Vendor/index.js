...
addReviews = async (data,vendorId) => {
  let { productId, reviewerName ,review} = data;
  let newObj = {
  ...
  let saveData = new Review(newObj)
...
console.log(result);
  if (result.status) {
      return helpers.showResponse(true, "Review Added", result.data._id, null, 200);
  }
  return helpers.showResponse(false, Messages.SERVER_ERROR, null, null, 200);
}
...
deleteReview = async (reviewId, vendorId) => {
  ...
  let response = await Review.update(
    { _id: reviewId, vendorId: vendorId },
    { $set: { status: 2 } }
  );
  if (response) {
    return helpers.showResponse(true, "Review Deleted", [], null, 200);
  }
  return helpers.showResponse(
    false,
    "Failed While Deleting Addon",
...
getReviews  = async (vendorId) => {
  ...
  let result = await getDataArray(Review, {vendorId: vendorId ,status: { $ne: 2 }}, '',null, null,'productId');
  if (result.status) {
    return helpers.showResponse(true, "Success", result.data, null, 200);
  }
  return helpers.showResponse(false, "No Data Found", null, null, 200);
